...
counter = 1
for filename in [el for el in os.listdir(settings.combined_filtered_dataset_dir_path) if el.endswith('.csv')]:
...
with open(f"{settings.combined_filter_test_results_dir_path}/scores.csv", "w+") as w_scores, open(f"{settings.combined_filter_test_results_dir_path}/params.csv", "w+") as w_params:
