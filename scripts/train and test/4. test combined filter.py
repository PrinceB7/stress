...
counter = 1
for filename in [el for el in os.listdir(settings.combined_filtered_dataset_dir) if el.endswith('.csv')]:
    ...
    params, scores = utils.participant_train_test_xgboost(participant=participant, train_dir=settings.combined_filtered_dataset_dir, m=settings.m, tune_parameters=True, model_dir=settings.not_filtered_model_dir)
...
with open(f"{settings.combined_filter_test_results_dir}/scores.csv", "w+") as w_scores, open(f"{settings.combined_filter_test_results_dir}/params.csv", "w+") as w_params:
