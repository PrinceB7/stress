...
counter = 1
for filename in [el for el in os.listdir(settings.behavioral_features_dataset_dir_path) if el.endswith('.csv')]:
    ...
    params, scores = utils.participant_train_test_xgboost(participant=participant, train_dir=settings.behavioral_features_dataset_dir_path, selected_features=settings.behavior_enhanced_selected_features, tuning=False)
...
with open(f"{settings.behavioral_data_test_results_dir_path}/scores.csv", "w+") as w_scores, open(f"{settings.behavioral_data_test_results_dir_path}/params.csv", "w+") as w_params:
