...
for participant in settings.participants:
    with open(f'{settings.ppg_filtered_dataset_dir_path}/{participant}.csv', 'w+') as w:
        ...
        for ground_truth in participants_ground_truths:
            ...
            while timestamp > till_timestamp:
                ...
                if selected_rr_intervals is not None and selected_ppg_values is not None:
                    try:
                        if utils.get_ppg_stdev(selected_ppg_values) < settings.ppg_thresholds[participant]:
...
# print stats for features - ppg filter
print('participant-email\t\tsamples\t\tstressed\tnot-stressed')
for participant in settings.participants:
    with open('{0}/{1}.csv'.format(settings.ppg_filtered_dataset_dir_path, participant), 'r') as r:
        lines = r.readlines()[1:]
