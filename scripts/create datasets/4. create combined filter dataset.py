...
for participant in settings.participants:
    with open(f'{settings.combined_filtered_dataset_dir_path}/{participant}.csv', 'w+') as w:
        ...
        for ground_truth in participants_ground_truths:
            ...
            while timestamp > till_timestamp:
                ...
                if selected_rr_intervals is not None and selected_ppg_values is not None and selected_acc_values is not None:
                    try:
                        if utils.get_ppg_stdev(ppg_values=selected_ppg_values) < settings.combined_thresholds['PPG'][participant] and not utils.is_acc_window_active(acc_values=selected_acc_values, activity_threshold=settings.combined_thresholds['ACC'][participant]):
...
for participant in settings.participants:
    with open('{0}/{1}.csv'.format(settings.acc_filtered_dataset_dir_path, participant), 'r') as r:
        lines = r.readlines()[1:]
